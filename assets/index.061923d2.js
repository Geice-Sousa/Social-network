...
function criarCadastro(email, password, name2) {
  return createUserWithEmailAndPassword(auth, email, password).then((usuaria) => updateProfile(usuaria.user, { displayName: name2 }));
}
function fazerLogin(email, password) {
  return signInWithEmailAndPassword(auth, email, password);
}
function observador(cb2) {
...
const iconeMao = "" + new URL("mulher-mao-com-uma-taca-de-vinho-transbordando-removebg-preview.cb863076.png", import.meta.url).href;
...
function pegaDados(querySnapshot) {
  let recebeDados = "";
...
const imgApresentacao = "" + new URL("background login.e44d675d.png", import.meta.url).href;
const iconeGoogle = "" + new URL("icone google.8edb2dab.png", import.meta.url).href;
const imagemFooter = "" + new URL("brinde_ta\xE7as-removebg-preview.45b0dd02.png", import.meta.url).href;
...
const login = () => {
  ...
  <div class="container-bcg-login">   
    <div class="bckgnd-txt-img">
      <img class="img-apresentacao" src="${imgApresentacao}" alt="dois cachos de uva diagonalmente opostos com o nome m\xE3es e vinhos entre ele">

      <h3 class="texto-apresentacao">Seja bem-vinda! M\xE3es e vinhos \xE9 uma rede social feita para m\xE3es que gostam de conversar sobre maternidade real acompanhadas de uma boa ta\xE7a de vinho. Por aqui a emo\xE7\xE3o \xE9 a flor da pele, com l\xE1grimas e risos garantidos. Esse \xE9 um espa\xE7o de conex\xE3o e partilha. <br> Junte-se a n\xF3s!</h3>
    </div>
    ...
    <div class="caixa-login">
      ...
      <section class="login-com">
        ...
        <p>Ainda n\xE3o possui cadastro? <br> <a href="/#cadastro">Crie sua conta!</a></p>
      </section>
    </div> 
  </div>

  <div class="footer-login-desktop">
    <img class="imagem-footer-login " src="${imagemFooter}" alt="duas m\xE3os pr\xF3ximas segurando ta\xE7as com vinho simulando um brinde">
  </div>
  `;
  ...
  btnGoogle.addEventListener("click", () => {
    loginComGoogle().then(() => {
      ...
      console.log("n\xE3o foi possivel concluir o login");
    });
  });
  return criarLogin;
};
const imgFundo = "" + new URL("background_mobile_preto-removebg-preview.bb25a132.png", import.meta.url).href;
const imgFundoDesktop = "" + new URL("background_desktop_preto-removebg-preview.a8264eee.png", import.meta.url).href;
const cadastro = () => {
  ...
  btnEnviar.addEventListener("click", (e) => {
    e.preventDefault();
    if (nome.value === "" || dataNascimento.value === "" || inputEmail === "" || inputSenha === "" || telefone === "" || filhx.value === "") {
      form.reportValidity();
    } else if (inputSenha.value.length < 6) {
      erroCadastro.innerHTML = "sua senha precisa ter mais de 6 dig\xEDtos";
    } else if (maiorDe18(dataNascimento.value) === false) {
      erroCadastro.innerHTML = "Infelizmente vc n\xE3o pode acessar essa plataforma/rede social, ela \xE9 destinada para maiores de 18 anos por fazer apologia e incentivar o uso de bebida alco\xF3lica";
    } else {
      criarCadastro(inputEmail.value, inputSenha.value, nome.value).then(() => {
        console.log("cadastrou");
        verificarEmail().then(() => {
          alert("Parab\xE9ns, seu cadastro foi realizado com sucesso! Agora basta fazer o login");
          console.log("email de confirma\xE7\xE3o enviado");
          window.location.hash = "";
          header.style.display = "block";
        });
      }).catch((error) => {
...
const db = uh(app);
function paraPostar(descricao) {
  const usuaria = dadosUsuaria();
  Xl(Ja(db, "postagens"), { descricao, ...usuaria });
}
function mostraPostAutomaticamente(postEnviado) {
  return tf(Ja(db, "postagens"), postEnviado);
}
function deletaPost(id2) {
  return Zl(Za(db, "postagens", id2));
}
function editaPost(id2) {
  return Kl(Za(db, "postagens", id2));
}
function atualizaEdicao(id2, texto) {
  Yl(Za(db, "postagens", id2), texto);
}
async function curtirPost(id2, curtidas) {
  await Yl(Za(db, "postagens", id2), {
    curtidas: Number(curtidas) + 1
  });
}
async function ordenaPosts() {
  const ordem = ol(Ja(db, "postagens"), dl("dataPostagem", "desc"));
  const paraOrdenar = await zl(ordem);
  return paraOrdenar;
}
const logoDesktop = "" + new URL("nome logo.29141619.png", import.meta.url).href;
const imgDesktop = "" + new URL("enchendo_ta\xE7a_de_vinho-removebg-preview.d8e56326.png", import.meta.url).href;
const postagem = () => {
  const header = document.querySelector(".header");
  ...
  <div class="imagem-postagem-desktop">
    <img class="logo-desktop" src="${logoDesktop}" alt="nome m\xE3es e vinhos logo">

    <img class="img-desktop" src="${imgDesktop}" alt="imagem  de garrafa derramand liquido em uma ta\xE7a">
  </div>
  ...
  <div class="container-txt">
    ...
    <div class="postagem">
      <div class="mensagem-ola">
        <p class="paragrafo">Ol\xE1 ${nomeUsuaria()}, seja bem-vinda! <br> O que voc\xEA deseja compartilhar?</p>
      </div>
    </div>
  ...
  btnPostar.addEventListener("click", () => {
    if (editar) {
      atualizaEdicao(id2, { descricao: novoTexto.value });
      editar = false;
      novoTexto.value = "";
    } else if (novoTexto.value === "") {
      digiteTexto.innerHTML = "Por favor, digite um texto";
    } else {
      paraPostar(novoTexto.value);
      novoTexto.value = "";
    }
  });
